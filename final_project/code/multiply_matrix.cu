...
py::array_t<float> global_matmul(py::array_t<float> a, py::array_t<float> b, py::array_t<float> c, int M, int N, int K){
    unsigned int sizeOfA = sizeof(float)*M*N;
    unsigned int sizeOfB = sizeof(float)*N*K;
    unsigned int sizeOfC = sizeof(float)*M*K;
    ...
    py::array_t<float> r_c = py::array_t<float>(M*K);
    for(int i = 0; i < M; i++){
        for(int j = 0; j < K; j++){
            r_c.mutable_at((i*M + j)) = h_c[(int)(i*M + j)];
        }
    }
